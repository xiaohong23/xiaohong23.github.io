...
<html><head><title>Microarray data for differential expression genes of short form sipa1</title></head>
<body background="background.jpg">
<br><font size=6><strong><center><p>Microarray Data of Expressing Short Form Sipa1</center></strong></font><br><br>
...
if ($selection=="PValue") {
	$data = mysql_query("select * from 3aweb where p_value <'$pvalue' and p_value!=0 order by p_value");
}
elseif ($selection=="Log2Diff") {
	$data = mysql_query("select * from 3aweb where abs(Diff_M) >'$log2' order by Diff_M");
}
elseif ($selection=="PValueLog") {
	$data = mysql_query("select * from 3aweb where abs(Diff_M) >'$log2' and p_value<'$pvalue' and p_value!=0 order by p_value");
}
